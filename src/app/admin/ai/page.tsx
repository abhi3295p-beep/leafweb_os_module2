...
export default async function AIPage() {
  ...
  const [totalTasks, completedTasks, failedTasks, pendingApprovals] = await Promise.all([
    prisma.aITask.count(),
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-16">
      <div className="mb-12">
        ...
        <p className="mt-2 text-mist">Manage AI-powered automation and workflows</p>
      </div>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-line bg-panel p-6">
          <p className="text-sm text-mist">Total AI Employees</p>
          <p className="mt-2 font-display text-3xl text-leaf">{aiEmployees.length}</p>
        </div>

        <div className="rounded-2xl border border-line bg-panel p-6">
          <p className="text-sm text-mist">Active Tasks</p>
          <p className="mt-2 font-display text-3xl text-leaf">
            {aiEmployees.reduce((acc, emp) => acc + emp.tasks.length, 0)}
          </p>
        </div>

        <div className="rounded-2xl border border-line bg-panel p-6">
          <p className="text-sm text-mist">Pending Approvals</p>
          <p className="mt-2 font-display text-3xl text-amber-400">{pendingApprovals}</p>
        </div>

        <div className="rounded-2xl border border-line bg-panel p-6">
          <p className="text-sm text-mist">Success Rate</p>
          <p className="mt-2 font-display text-3xl text-leaf">{successRate}%</p>
        </div>
      </div>

      {aiEmployees.length === 0 ? (
        <div className="rounded-3xl border border-line bg-panel p-12 text-center">
          <p className="text-mist">No AI employees configured yet.</p>
          <Link
            href="/admin/ai/new"
        ...
        <div className="space-y-6">
          {aiEmployees.map((employee) => {
            ...
            return (
              <Link key={employee.id} href={`/admin/ai/${employee.id}`}>
                <Card className="cursor-pointer transition-all hover:border-leaf hover:bg-panel/80">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <CardDescription>
                        <span className="block text-leaf">{employee.type}</span>
      ...
      <div className="mt-12">
        <h2 className="font-display text-2xl text-foam">Task Summary</h2>
        ...
        <div className="mt-6 rounded-2xl border border-line bg-panel p-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-line pb-4 text-sm">
              ...
              <span className="font-semibold text-foam">{totalTasks}</span>
            </div>
            ...
            <div className="flex items-center justify-between border-b border-line pb-4 text-sm">
              <span className="text-mist">Failed</span>
              <span className="font-semibold text-rose-400">{failedTasks}</span>
            </div>

            <div className="flex items-center justify-between text-sm">
              <span className="text-mist">Pending Approvals</span>
              <span className="font-semibold text-amber-400">{pendingApprovals}</span>
            </div>
          </div>
        </div>
      </div>
